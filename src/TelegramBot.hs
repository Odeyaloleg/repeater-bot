    ...
    TelegramUpdates updates -> do
      ...
      if length botMsgs == 0
        then logDebug "There is no handled updates."
          ...
          if failedSize > 0
            then logRelease $ BSL8.concat ["Failed to send ",(BSL8.pack $ show failedSize),"/",(BSL8.pack $ show succeedSize)," messages."]
            else logDebug $ BSL8.concat ["Successfully sent all ",(BSL8.pack $ show succeedSize)," messages."]
