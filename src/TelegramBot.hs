...
runTelegramBot d lastUpdId = do
  ...
  case recievedUpdates of
    BadRequest description -> lift $ putStrLn $ "TelegramBot error: " ++ description
    TelegramUpdates updates -> do
      (HandlerData x y d) <- handleUpdates updates (HandlerData 0 0 d)
      if x == 0
      then lift $ putStrLn $ "Telegram: Didn't get any messages possible for handling."
      else lift $ putStrLn $ "Telegram: Handled " ++ show x ++ "/" ++ show (length updates) ++ " messages."
      runTelegramBot d y
...
handleUpdates :: [TelegramMsgUpdate] -> HandlerData -> ReaderT TelegramSettings IO HandlerData
...
handleUpdates ((TelegramMsgUpdate updateId chatId update):rest) (HandlerData succeedAnswersSize lastUpdateId d) = do
  ...
  case (update,isAskedForRepetitions) of
    (TextMsg textMsg _, True) -> do
      ...
      case result of
        Nothing -> do
          lift $ sendAnswerNTimes 1 "sendMessage" (requestSettings s) (TelegramMsgJSON chatId Nothing "Incorrent answer. Please choose one of the buttons or input digit from 1 to 5." Nothing)
          handleUpdates rest (HandlerData (succeedAnswersSize + 1) updateId usersData)
        Just n  -> do
          lift $ sendAnswerNTimes 1 "sendMessage" (requestSettings s) (TelegramMsgJSON chatId Nothing ("I will repeat your messages " ++ show n ++ " times.") (Just TelegramKBRemove))
          handleUpdates rest (HandlerData (succeedAnswersSize + 1) updateId (M.insert chatId (False, n) d))
    (_, True) -> do
      lift $ sendAnswerNTimes 1 "sendMessage" (requestSettings s) (TelegramMsgJSON chatId Nothing "Incorrent answer. Please choose one of the buttons or input digit from 1 to 5." Nothing)
      handleUpdates rest (HandlerData (succeedAnswersSize + 1) updateId usersData)
    (TextMsg textMsg entities, _) ->
      if head textMsg == '/'
        then do
          ...
          if textMsg == "/repeat"
            then handleUpdates rest (HandlerData (succeedAnswersSize + 1) updateId (M.insert chatId (True, repetitionsNum) usersData))
            else handleUpdates rest (HandlerData (succeedAnswersSize + 1) updateId usersData)
        else do
          lift $ sendAnswerNTimes repetitionsNum "sendMessage" (requestSettings s) (composeTextMsgRequest chatId textMsg entities)
          handleUpdates rest (HandlerData (succeedAnswersSize + 1) updateId usersData)
    (StickerMsg fileId, _) -> do
      lift $ sendAnswerNTimes repetitionsNum "sendSticker" (requestSettings s) (TelegramStickerJSON chatId fileId)
      handleUpdates rest (HandlerData (succeedAnswersSize + 1) updateId usersData)
    (UnknownMsg, _) -> do
