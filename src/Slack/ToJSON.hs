...
instance ToJSON SlackChallengeJSON where
  toJSON (SlackChallengeJSON challenge) = object ["challenge" .= challenge]

data SlackTextMessageJSON =
  SlackTextMessageJSON String String

instance ToJSON SlackTextMessageJSON where
  toJSON (SlackTextMessageJSON channel textMessage) =
    object ["channel" .= channel, "text" .= textMessage]

data SlackCommandAnswerJSON
  = TextAnswerJSON String
  | ButtonsAnswerJSON String

instance ToJSON SlackCommandAnswerJSON where
  toJSON (TextAnswerJSON textMessage) =
    object ["text" .= textMessage, "response_type" .= ("ephemeral" :: String)]
    ...
    object
      ...
      , "blocks" .=
        [ object
            [ "type" .= ("section" :: String)
            , "text" .=
              object ["type" .= ("mrkdwn" :: String), "text" .= textMessage]
            ]
        , object
            [ "type" .= ("actions" :: String)
            , "elements" .=
